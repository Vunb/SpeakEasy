...
tf.app.flags.DEFINE_string("model_type", "embedding_attention", 
                           "Seq2Seq model type: 'embedding_attention' or 'embedding'")
...
tf.app.flags.DEFINE_boolean("self_test", False,
                            "Set to True to run a self-test.")
...
def read_data(data_path, max_size=None):
  ...
  with gfile.GFile(data_path, mode="r") as data_file:    
    ...
    while prompt and response and (not max_size or counter < max_size):
      # Skip empty lines
      # Scripts are punctuated with 'breakHerePlease' to prevent pairing lines from different movies or episodes
      if (len(prompt.split()) > 0) and (len(response.split()) > 0) and \
         ('breakHerePlease' not in prompt.split()) and ('breakHerePlease' not in response.split()):
        counter += 1
        if counter % 1000 == 0:
...
def create_model(session, forward_only):
  ...
  if FLAGS.buckets:
    ...
    model = seq2seq_model.Seq2SeqModel(FLAGS.vocab_size, FLAGS.max_sentence_length, FLAGS.size, FLAGS.num_layers, FLAGS.max_gradient_norm, FLAGS.batch_size, FLAGS.learning_rate, FLAGS.learning_rate_decay_factor, FLAGS.model_type, forward_only=forward_only)
  ...
  with tf.Session() as sess:
    ...
    while sentence:
      ...
      if FLAGS.buckets:
        ...
        encoder_inputs, decoder_inputs, target_weights = model.get_batch(
            {bucket_id: [(token_ids, [])]}, bucket_id=bucket_id)
        sys.stdout.flush()
        # Get output logits for the sentence.
        _, _, _, output_logits = model.step(sess, encoder_inputs, decoder_inputs,
                                         target_weights, True, bucket_id=bucket_id)
      else:
        encoder_inputs, decoder_inputs, target_weights = model.get_batch([(token_ids, [])], bucket_id=None)
        _, _, _, output_logits = model.step(sess, encoder_inputs, decoder_inputs,
                                       target_weights, True, bucket_id=None)
...
def self_test():
  ...
  with tf.Session() as sess:
    ...
    for _ in xrange(5):  # Train the fake model for 5 steps.
      ...
      encoder_inputs, decoder_inputs, target_weights = model.get_batch(
          data_set, bucket_id=bucket_id)
      model.step(sess, encoder_inputs, decoder_inputs, target_weights, True,
                 bucket_id=bucket_id)
